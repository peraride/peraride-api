...
exports.locate = async (req, res, next) => {
  try {
    ...
    Station.findOne({station_id: req.query.station_id}).  populate({
      path: 'docks',
...
exports.unlock = async (req, res, next) => {
  try {
    ...
    if(Object.keys(req.query).length === 0) throw new APIError(`Dock ID Field can not be empty`, httpStatus.NOT_FOUND)
    ...
    if(dock.dock_state === 'FREE')  throw new APIError(`Dock is Empty !`, httpStatus.NOT_FOUND)
      
    const bike = await Bike.findById(dock.bike)
    if (!bike) throw new APIError(`No Bike associated with ${bikeID}`, httpStatus.NOT_FOUND)
...
exports.lock = async (req, res, next) => {
  try {
    ...
    throw new APIError(`dock_id and rfid_code must be provided`, httpStatus.NOT_FOUND)
    ...
    await Dock.findOne({dock_id: dock_id}, async function(err,dock){
      ...
      try {
        ...
        if(dock.dock_state  == 'FREE'){
          ...
          await Bike.findOne({rfid_code: rfid_code}, async function(err,bike){
            
            try {
        
              console.log('Dock eka hari, bike ekath dan hari')

             if (err) throw new APIError(`No Bike associated with the given RFID_CODE ${rfid_code}`, httpStatus.NOT_FOUND)
              
      
             const user = await User.findById(bike.current_user);

             if(!user) throw new APIError(`No User associated with user_id ${bike.current_user}`, httpStatus.NOT_FOUND)
            ...
            throw new APIError(`Dock Must be free in order to dock a bike`, httpStatus.NOT_FOUND)
