...
exports.update = async (req, res, next) => {
    try {
        if (Object.keys(req.query).length === 0)
            throw new APIError(`Bike ID Field can not be empty`, httpStatus.NOT_FOUND);
    ...
    try {
        ...
        await Bike.findOne({ bike_id: bikeID }, async function (err, bike) {
            try {
                if (!bike)
                    throw new APIError(
                        `No bike associated with Bike ID ${bikeID}`,
    ...
    try {
        ...
        await Bike.findOne({ bike_id: bike_id }, function (err, bike) {
            try {
                if (!bike)
                    throw new APIError(
                        `No bike associated with Bike ID ${bike_id}`,
                        httpStatus.NOT_FOUND
                    );
                res.status(httpStatus.OK);
                res.send(bike.transform());
